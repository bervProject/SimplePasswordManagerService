...
export default {
  before: {
    ...
    create: [required('name', 'email', 'password'), hashPassword('password'), userAuditHook()],
    update: [disallow()],
    patch: [preventChanges(false, 'email', 'role'), iff(hashWhenAvailable, hashPassword('password')), authenticate('jwt'), userAuditHook()],
